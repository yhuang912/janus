...
class MinMaxRedundancy
{
  ...
  {
    public int hcode;
    public HashKey(AstNode n) { hcode = getHashCode(n); }
    ...
    { if(node.getType()!=AstAddNode.class) return super.hashCode();
  ...
  /*
     Checks if either of the two nodes make the other redundant
     as a child of the same Min/Max node. Right now, works only on
     Add nodes. NOTE: HashKey needs to be updated whenever
     this one is.
     */
  public static Result checkRedundant(AstNode node1,AstNode node2)
  {
    ...
    if(addchild1.length!=addchild2.length) return Result.needboth;

    int k, len = addchild1.length;
    for(k=0;k<len-1;++k) if(!addchild1[k].equals(addchild2[k])) break;
    if(k<len-1) return Result.needboth;
    ...
    if(last1.getType()!=AstValueNode.class 
        || last2.getType()!=AstValueNode.class) return Result.needboth;
    ...
    if(v1>v2) return Result.small2;
    else return Result.small1;
  }

  public static AstNode[] dumpToArray 
    (HashMap<HashKey, LinkedList<AstNode>> hmap, int len)
  {
    java.util.Iterator<java.util.Map.Entry<HashKey,LinkedList<AstNode>>> it
      = hmap.entrySet().iterator();
    int j=0;
    AstNode[] dest = new AstNode[len];
    while(it.hasNext())
    { java.util.ListIterator<AstNode> src = it.next().getValue().listIterator();
      while(src.hasNext())
      { AstNode t = src.next();
        if(t!=null) dest[j++]=t;
      }
