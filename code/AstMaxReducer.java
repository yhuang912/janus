...
public class AstMaxReducer
{
  ...
  public AstMaxReducer(AstReducer r) { reducerMain=r; }
...
//      if(children[i].getType()==node.getType()) return true;
    ...
    boolean hasconst = children.length>scount && curmax>nodeinfo.lowerLim;
    scount = 0;
    ...
    if(hasconst) newchildren[scount] = AstValueNode.create(curmax);
  ...
  private boolean exRemoveRedundantMaxChildren
    (AstNode node, AstReducer.ReduceInfo nodeinfo)
  {
    if(node.getType()!=AstMaxNode.class) return false;
    ...
    { MinMaxRedundancy.HashKey rk 
        = new MinMaxRedundancy.HashKey(maxchild[i]);
      if(hmap.containsKey(rk)) 
        if(checkRedundantAndAdd(hmap.get(rk),maxchild[i])) nullcount++;
      else
    ...
    if(maxchild.length==hmap.size()) return false;
    if(nullcount==0) return false;

    AstNode newmaxchild[] 
      = MinMaxRedundancy.dumpToArray(hmap,maxchild.length-nullcount);
