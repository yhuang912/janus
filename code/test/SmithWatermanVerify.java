...
public class SmithWatermanVerify
{
  ...
  {
    SmithWaterman sw = new SmithWaterman(new String(dataA),
        new String(dataB),cost);
  ...
  private char[] fillRandom(char[] input)
  {
    ...
    { if(input[i]=='#') rv[i]=(char)(random.nextInt()%4+'A');
      else rv[i]=input[i];
    }
    return rv;
  }
  ...
  int evaluateGraph(AstNode node,char[] sampA, char[] sampB)
  {
    ...
    { AstNode[] child = node.children();
      rv=0;
      for(int i=0;i<child.length;++i) rv+=evaluateGraph(child[i],sampA,sampB);
  ...
  {
    assert cost.length>=sampA.length && cost.length>=sampB.length
      : "cost[] array too small for SmithWaterman evaluation";

    int dp[][] = new int[sampA.length+1][sampB.length+1];
    dp[0][0]=0;
    for(int i=1;i<=sampA.length;++i) dp[i][0]=0;
    for(int j=1;j<=sampB.length;++j) dp[0][j]=0;
    for(int i=1;i<=sampA.length;++i) for(int j=0;j<=sampB.length;++j)
    { dp[i][j]=0;
      int x=dp[i-1][j-1]+AstSWSimilarityNode.evalChar(sampA[i-1],sampB[j-1]);
      if(x>0) dp[i][j]=x;
      for(int k=0;k<i;++k)
      { x=dp[k][j]+cost[i-k-1];
