...
//		ccs = new OptEDCORE_3Lplus2S_L[EditDistanceCommon.bitLength(Math.max(
//				sdnaLen, cdnaLen))];
		...
		for (int l = 2; l <= sdnaLen + cdnaLen; l++) {
			for (int m = 1; m <= l; m++) {
				...
				if (i >= 1 && i <= sdnaLen && j >= 1 && j <= cdnaLen) {
					...
					System.arraycopy(D[i - 1][j - 1].values, 0, vals,
							sigma * 2, D[i - 1][j - 1].width);
					System.arraycopy(D[i - 1][j - 1].invd, 0, invd, sigma * 2,
							D[i - 1][j - 1].width);
					System.arraycopy(D[i - 1][j - 1].lbls, 0, lbls, sigma * 2,
							D[i - 1][j - 1].width);
					if (D[i - 1][j - 1].width < W) { // it must be the case that
														// (D[i-1][j-1].wdith +
														// 1 == W)
						vals[W + sigma * 2 - 1] = 0;
					}

					System.arraycopy(D[i][j - 1].values, 0, vals,
							W + sigma * 2, D[i][j - 1].width);
					System.arraycopy(D[i][j - 1].invd, 0, invd, W + sigma * 2,
							D[i][j - 1].width);
					System.arraycopy(D[i][j - 1].lbls, 0, lbls, W + sigma * 2,
							D[i][j - 1].width);
					if (D[i][j - 1].width < W) { // it must be the case that
													// (D[i][j-1].wdith + 1 ==
													// W)
						vals[2 * W + sigma * 2 - 1] = 0;
					}

					System.arraycopy(D[i - 1][j].values, 0, vals, 2 * W + sigma
							* 2, D[i - 1][j].width);
					System.arraycopy(D[i - 1][j].invd, 0, invd, 2 * W + sigma
							* 2, D[i - 1][j].width);
					System.arraycopy(D[i - 1][j].lbls, 0, lbls, 2 * W + sigma
							* 2, D[i - 1][j].width);
					if (D[i - 1][j].width < W) { // it must be the case that
													// (D[i-1][j].wdith + 1 ==
													// W)
						vals[3 * W + sigma * 2 - 1] = 0;
					}

					EditDistanceCommon.ccs[W - 1].startExecuting(vals, invd,
							lbls);
