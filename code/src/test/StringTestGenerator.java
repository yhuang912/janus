...
public class StringTestGenerator {
	...
	public StringTestGenerator(helper hper) {
		testUser = hper;
	}

	public helper getTestUser() {
		return testUser;
	}

	public void setTestUser(helper x) {
		testUser = x;
	}
	...
	public void setStringLen(int x) {
		defaultlen = x;
	}

	public float getSecretPercentLo() {
		return secretPercentLo;
	}

	public void setSecretPercentLo(int x) {
		secretPercentLo = x;
	}

	public float getSecretPercentHi() {
		return secretPercentHi;
	}

	public void getSecretPercentHi(int x) {
		secretPercentHi = x;
	}

	private float getSecretPercentStep() {
		return secretPercentStep;
	}

	private void getSecretPercentStep(int x) {
		secretPercentStep = x;
	}
	...
	public void singleMutationTest(Eraser eraser, PrintStream printStream,
			float mutationRate) {
	...
	public void singleBlockMutationTest(Eraser eraser, PrintStream printStream,
			float mutationRate) {
		final int len = defaultlen;
		final float lo = secretPercentLo, hi = secretPercentHi, step = secretPercentStep;
		for (float secretPercent = lo; secretPercent <= hi; secretPercent += step) {
	...
	public void randomInsDel(Eraser eraser, PrintStream printStream,
		...
		assert mutationCount != 0 || charsMutated == 0 : "avgInsDelLen is too long for given mutationRate";
		/*
		...
		for (float secretPercent = lo; secretPercent <= hi; secretPercent += step) {
			...
			int[] delLenA = nonZeroRandomPartitions(charsMutated,
					mutationCount / 2);
			int[] delLenB = nonZeroRandomPartitions(charsMutated,
					mutationCount / 2);
			index2Len(charsMutated, delLenA);
			index2Len(charsMutated, delLenB);
			for (int i = 0; i < mutationCount / 2 - 1; ++i) // we may have a
															// tail of large sub
			{
			 ...
			 * System.out.print(delLenA[i]+" "); System.out.println(""); for(int
			 * i=0;i<delLenB.length;++i) System.out.print(delLenB[i]+" ");
			 * System.out.println("");
			 */
	...
	/*
	 * Randomly partitions the set of integers in [0,len] into partitionCount
	 * partitions, and returns the indices where partitions start. First return
	...
	public final Eraser singleBlockEraser = new Eraser() {
		public void erase(char[] data, float percent) {
			...
			for (int i = start; i < start + eraseCount; ++i)
				data[i] = '#';
		}
	};
	...
	public Eraser nBlockEraser(final int n) {
		return new Eraser() {
			public void erase(char[] data, float percent) {
				...
				int origLens[] = withZeroRandomPartitions(data.length
						- eraseCount, n + 1);
				...
				for (int i = 0; i < eraseLens.length; ++i) {
					...
					if (i < eraseLens.length)
						start += eraseLens[i] + origLens[i];
				}
			}
		};
	}
