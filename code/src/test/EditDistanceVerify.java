...
public class EditDistanceVerify implements StringTestGenerator.helper {
	...
	private char[] fillRandom(char[] input) {
		...
		for (int i = 0; i < input.length; ++i)
			rv[i] = (input[i] == '#' ? (char) (random.nextInt(4) + 'A')
					: input[i]);
		return rv;
	}
	...
	private int evaluateTree(AstNode node, char[] sampA, char[] sampB) {
		...
		if (node.getType() == AstValueNode.class)
			...
			for (int i = 0; i < child.length; ++i)
				rv += evaluateTree(child[i], sampA, sampB);
	...
	private int manualEd(char[] dataA, char[] dataB) {
		...
		for (a = 0; a <= dataA.length; ++a)
			dp[a][0] = a;
		for (b = 0; b <= dataB.length; ++b)
			dp[0][b] = b;
		for (a = 1; a <= dataA.length; ++a)
			for (b = 1; b <= dataB.length; ++b) {
				if (dataA[a - 1] == dataB[b - 1])
					dp[a][b] = dp[a - 1][b - 1];
				else if (dp[a - 1][b - 1] < dp[a][b - 1]
						&& dp[a - 1][b - 1] < dp[a - 1][b])
					dp[a][b] = dp[a - 1][b - 1] + 1;
