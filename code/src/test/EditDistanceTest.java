...
public class EditDistanceTest {
	private static StringTestGenerator.helper timePrinter = new StringTestGenerator.helper() {
		public void useTest(char[] testA, char[] testB, float dataLabel,
			...
			EditDistance ed = new EditDistance(new String(testA), new String(
					testB));
			end = System.nanoTime();
			printStream.println(dataLabel + ": " + (end - start) * 1e-6);
		}
	};
	private static StringTestGenerator.helper testPrinter = new StringTestGenerator.helper() {
		public void useTest(char[] testA, char[] testB, float dataLabel,
				PrintStream printStream) {
			printStream.println("A: " + new String(testA));
			printStream.println("B: " + new String(testB));
			...
			// printStream.println("");
			printStream.println(dataLabel + ": "
					+ AstNodeCounter.count(ed.getRoot()));
		}
	};
	private static StringTestGenerator gen = new StringTestGenerator(
			testPrinter);
	...
	public static void parseCommandLine(String[] args) {
		for (int i = 0; i < args.length; ++i) {
			if (args[i].startsWith("-deflen="))
				gen.setStringLen(Integer.parseInt(args[i].substring("-deflen="
						.length())));
			if (args[i].equals("-scatrErase"))
				curEraser = gen.scatterEraser;
			if (args[i].startsWith("-nblockErase="))
				curEraser = gen.nBlockEraser(Integer.parseInt(args[i]
						.substring("-nblockErase=".length())));
		}
	}
