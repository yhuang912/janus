...
public class AstTraverser {
	...
	public static class NodeData {
		...
		public void calculateFromLeaf(AstNode leaf) {
			if (leaf.getType() == AstValueNode.class)
				...
				this.nodeCount = 3; // leaf representing comparison between
									// chars
			else
	...
	private static NodeData traverseSubtree(AstNode node,
		...
		for (--i; i >= 0; --i) {
			data[i] = new NodeData();
			data[i].calculateFromChildren(node, traverseSubtree(children[i],
					visited), last);
			last = data[i];
		}
