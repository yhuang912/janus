...
public class AstMinReducer {
	...
	public AstMinReducer(AstReducer r) {
		reducerMain = r;
	}
	...
	public boolean reduce(AstNode node, AstReducer.ReduceInfo nodeinfo) {
		...
		/*
		 * AstNode[] children = AstReducer.flattenedChildList(node) .toArray(new
		 * AstNode[0]); assert children.length>0; // release references to old
		 * data node.setData(new AstMinNode(children));
		 */
		...
		if (hasconst)
			newchildren[scount] = AstValueNode.create(curmin);
	...
	/**
	 * Tries to remove redundant arguments to AstMinNode, returns true iff some
	 * change was made to node. For now, it basically does this:
	 * <code>min(add(x,1),add(x,2),y) --&gt; min(add(x,1),y)</code>
	 */
	private boolean exRemoveRedundantMinChildren(AstNode node,
		...
		if (minchild.length > branchmax)
			branchmax = minchild.length;
		branchtotal += minchild.length;
		branchcount++;
