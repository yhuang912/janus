...
public class AstAddReducer {
	...
	public AstAddReducer(AstReducer r) {
		reducerMain = r;
	}
	...
	public boolean reduce(AstNode node, AstReducer.ReduceInfo nodeinfo) {
		...
		AstNode[] children = AstReducer.flattenedChildList(node).toArray(
				new AstNode[0]);
		...
		for (int i = 0; i < children.length; ++i) {
			if (children[i].getType() != AstValueNode.class) {
				...
				cursum += ((AstValueNode) children[i].getData()).getValue();
	...
	private boolean distributeConstAddThroughMin(AstNode node) {
		...
		if (child[child.length - 1].getType() != AstValueNode.class)
			return false;
		for (int i = 0; i < child.length - 1; ++i) {
			if (child[i].getType() != AstMinNode.class)
				return false;
			AstNode minchild[] = child[i].children();
			for (int j = 0; j < minchild.length; ++j) {
				if (minchild[j].getType() != AstAddNode.class)
					return false;
		...
		int incrAmount = ((AstValueNode) child[child.length - 1].getData())
				.getValue();
		for (int i = 0; i < child.length - 1; ++i) // for each min in add
		{
			...
			for (int j = 0; j < minchild.length; ++j) // for each add in min
			{
				...
				for (int k = 0; k < addchild.length; ++k)
					newaddchild[k] = addchild[k];
				AstNode lastchild = addchild[addchild.length - 1];
				newaddchild[addchild.length - 1] = AstValueNode
						.create(incrAmount
								+ ((AstValueNode) lastchild.getData())
										.getValue());
		...
		if (newchild.length > 1)
			node.setData(new AstAddNode(newchild));
		else
			node.setData(newchild[0].getData());
	...
	/*
	 * Opposite of distributeConstAddThroughMin Factors out variable parts
	 * add(min(add(x,y,z),add(x,y,w)),p,q) --> add(x,y,min(z,w),p,q) Helps out
	 * limit checking for the terms that remain unfactored
	 */
	private boolean factorAddThroughMin(AstNode node,
		...
		for (int i = 0; i < addchild.length; ++i)
			if (addchild[i].getType() == AstMinNode.class) {
				...
				AstNode[] factored = factorAddsOut(minnode.children(),
						newminchild);
				...
				for (int j = 0; j < factored.length; ++j)
					newaddchild.add(factored[j]);
				newaddchild.add(AstMinNode.create(newminchild
						.toArray(new AstNode[0])));
				sthFactored = true;
			} else
				newaddchild.add(addchild[i]);
	...
	private AstNode[] factorAddsOut(AstNode[] addnode,
		...
		for (i = 0; i < addnode.length; ++i)
			rest.add(new ArrayList<AstNode>(Arrays
					.asList(addnode[i].children())));
		...
		// sort all
		for (i = 0; i < addnode.length; ++i)
			java.util.Collections.sort(rest.get(i), cmp);
		...
		do {
			...
			for (i = 1; i < addnode.length; ++i) {
				...
				if (cmp.compare(min, cur) > 0)
					min = cur;
				if (cmp.compare(max, cur) < 0)
					max = cur;
			}
			if (cmp.compare(min, max) == 0) {
				isect.add(min);
				for (i = 0; i < addnode.length; ++i)
					rest.get(i).set(next[i]++, null);
			} else
				for (i = 0; i < addnode.length; ++i)
					if (cmp.compare(rest.get(i).get(next[i]), max) != 0)
						next[i]++;

			for (i = 0; i < addnode.length; ++i)
				if (next[i] >= rest.get(i).size()) {
					done = true;
					break;
				}
		} while (!done);
		...
		for (i = 0; i < addnode.length; ++i) {
			...
			for (j = 0; j < rest.get(i).size(); ++j)
				if (rest.get(i).get(j) != null)
