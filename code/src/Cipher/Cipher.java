...
public final class Cipher {
	...
	public static BigInteger encrypt(BigInteger lp0, BigInteger lp1, int k,
			BigInteger m) {
	...
	public static BigInteger decrypt(BigInteger lp0, BigInteger lp1, int k,
			BigInteger c) {
		BigInteger ret = getPadding(lp0, lp1, k);
		ret = ret.xor(c);
	...
	// the following three functions are dedicated for implementing negation
	// gate
	...
	public static BigInteger encrypt(int j, BigInteger key, BigInteger msg,
			int msgLength) {
		return msg.xor(getPaddingOfLength(j, key, msgLength));
	}

	public static BigInteger decrypt(int j, BigInteger key, BigInteger cph,
			int cphLength) {
		return cph.xor(getPaddingOfLength(j, key, cphLength));
	}

	private static BigInteger getPaddingOfLength(int j, BigInteger key,
			int padLength) {
