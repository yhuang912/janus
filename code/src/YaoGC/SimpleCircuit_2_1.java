...
public abstract class SimpleCircuit_2_1 extends Circuit {
	...
	private void debugPrint() {
		...
		if (this instanceof G_AND_2_1 || this instanceof G_OR_2_1) {
			...
			if (inWireL.value == Wire.UNKNOWN_SIG)
				System.out.print("("
						+ ((!inWireL.invd) ? (inWireL.lbl + ", " + inWireL.lbl
								.xor(Wire.R.shiftLeft(1).setBit(0)))
								: (inWireL.lbl.xor(Wire.R.shiftLeft(1)
										.setBit(0))
										+ ", " + inWireL.lbl)) + ") " + gtype
						+ " ");
			else
			...
			if (inWireR.value == Wire.UNKNOWN_SIG)
				System.out.print("("
						+ ((!inWireR.invd) ? (inWireR.lbl + ", " + inWireR.lbl
								.xor(Wire.R.shiftLeft(1).setBit(0)))
								: (inWireR.lbl.xor(Wire.R.shiftLeft(1)
										.setBit(0))
										+ ", " + inWireR.lbl)) + ") = ");
			else
			...
			if (outWire.value == Wire.UNKNOWN_SIG)
				System.out.print("("
						+ ((!outWire.invd) ? (outWire.lbl + ", " + outWire.lbl
								.xor(Wire.R.shiftLeft(1).setBit(0)))
								: (outWire.lbl.xor(Wire.R.shiftLeft(1)
										.setBit(0))
										+ ", " + outWire.lbl)) + ").");
			else
